    ...
    var student_module = {
        ...
        batch_exists: function (batch_name, callBack) {
            try {
                ...
                mongo.connect(url, { useNewUrlParser: true }, function (err, db) {
                    ...
                    var cursor = db.db().collection(dbb.USER).find({ "batch_name": batch_name });
        ...
        add_batch: function (new_batch, callBack) {
            try {
                mongo.connect(url, { useNewUrlParser: true }, function (err, db) {
                    assert.equal(null, err);
                    db.db().collection(dbb.USER).insertOne(new_batch, function (err, result) {
                        if (err) {
        ...
        update_student: function (id, batch_name , batch_type) {
            try {
                mongo.connect(url, { useNewUrlParser: true }, function (err, db) {
                    assert.equal(null, err);
                ...
                mongo.connect(url, { useNewUrlParser: true }, function (err, db) {
                    ...
                    cursor.forEach(function (doc, err) {
                        ...
                        if (batch.length == 0) {
                            ...
                            callBack(batch, false, "Students Found");
                        }
                        db.close();
                    })
                })
